...
/**
 * A {@link Map} wrapper that implements {@link ObservableMap}.
 *
 * <p>This is a package-private implementation that wraps a standard {@link Map} and adds
 * observability. All mutating operations (put, remove, clear, etc.) fire
 * {@link MapChangeListener.Change} events to registered listeners.</p>
 *
 * <p>The wrapper provides observable views for {@link #keySet()}, {@link #values()}, and
 * {@link #entrySet()}. Modifications through these views (including their iterators) also
 * trigger proper change notifications.</p>
 *
 * @param <K> the type of keys maintained by this map
 * @param <V> the type of mapped values
 * @see ObservableMap
 * @see MapChangeListener
 * @see JuxCollections#observableHashMap()
 */
class ObservableMapWrapper<K, V> implements ObservableMap<K, V> {
    ...
    /**
     * Creates a new observable wrapper around the specified backing map.
     *
     * <p>The backing map is used directly (not copied), so any external modifications
     ...
     *
     * @param map the backing map to wrap, must not be {@code null}
     */
    ...
    /**
     * A concrete change event describing a single put or remove operation on this map.
     */
