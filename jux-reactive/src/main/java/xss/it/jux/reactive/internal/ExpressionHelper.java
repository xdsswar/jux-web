...
/**
 * Core listener management engine for the JUX reactive system.
 *
 * Manages invalidation and change listeners for observable values using
 * three internal strategies to minimize overhead:
 * <ul>
 *   <li>{@code SingleInvalidation} — optimized for a single invalidation listener</li>
 *   <li>{@code SingleChange} — optimized for a single change listener</li>
 *   <li>{@code Generic} — handles multiple mixed listeners</li>
 * </ul>
 *
...
public abstract class ExpressionHelper<T> extends ExpressionHelperBase {
    ...
    ExpressionHelper(ObservableValue<T> observable) {
        this.observable = observable;
    }

    public static <T> ExpressionHelper<T> addListener(ExpressionHelper<T> helper,
            ObservableValue<T> observable, InvalidationListener listener) {
        if ((observable == null) || (listener == null)) {
            throw new NullPointerException();
        }
        observable.getValue(); // validate
        return (helper == null) ? new SingleInvalidation<>(observable, listener)
    ...
    public static <T> ExpressionHelper<T> addListener(ExpressionHelper<T> helper,
            ObservableValue<T> observable, ChangeListener<? super T> listener) {
        if ((observable == null) || (listener == null)) {
            throw new NullPointerException();
        }
        observable.getValue(); // validate
        return (helper == null) ? new SingleChange<>(observable, listener)
                : helper.addListener(listener);
    }
    ...
    // ═══════════════════════════════════════════════════════════════
    //  Strategy 1: Single InvalidationListener
    // ═══════════════════════════════════════════════════════════════
    ...
    private static class SingleInvalidation<T> extends ExpressionHelper<T> {
        ...
        private SingleInvalidation(ObservableValue<T> observable, InvalidationListener listener) {
            super(observable);
        ...
        protected ExpressionHelper<T> addListener(ChangeListener<? super T> listener) {
            return new Generic<>(observable, this.listener, listener, observable.getValue());
        }
    ...
    // ═══════════════════════════════════════════════════════════════
    //  Strategy 2: Single ChangeListener
    // ═══════════════════════════════════════════════════════════════
    ...
    private static class SingleChange<T> extends ExpressionHelper<T> {
        ...
        @Override
        protected ExpressionHelper<T> addListener(InvalidationListener listener) {
            return new Generic<>(observable, listener, this.listener, this.currentValue);
        }
        ...
        @Override
        protected ExpressionHelper<T> addListener(ChangeListener<? super T> listener) {
            return new Generic<>(observable, this.listener, listener, this.currentValue);
        }
    ...
    // ═══════════════════════════════════════════════════════════════
    //  Strategy 3: Generic (multiple mixed listeners)
    // ═══════════════════════════════════════════════════════════════
    ...
    private static class Generic<T> extends ExpressionHelper<T> {
        ...
        private Generic(ObservableValue<T> observable,
                InvalidationListener listener0, InvalidationListener listener1) {
            super(observable);
            this.invalidationListeners = new InvalidationListener[]{listener0, listener1};
            this.invalidationSize = 2;
            this.changeListeners = new ChangeListener[0];
            this.changeSize = 0;
            this.currentValue = observable.getValue();
        }
        ...
        private Generic(ObservableValue<T> observable,
                ...
                ChangeListener<? super T> changeListener, T currentValue) {
            super(observable);
        ...
        private Generic(ObservableValue<T> observable,
                ...
                ChangeListener<? super T> listener1, T currentValue) {
            super(observable);
            this.invalidationListeners = new InvalidationListener[0];
        ...
        protected Generic<T> addListener(InvalidationListener listener) {
            if (invalidationListeners.length == invalidationSize) {
                invalidationListeners = Arrays.copyOf(invalidationListeners,
                        invalidationSize + (invalidationSize >> 1) + 1);
            }
                ...
                for (int index = 0; index < invalidationSize; index++) {
                    if (listener.equals(invalidationListeners[index])) {
                        if (invalidationSize == 1) {
                            ...
                            return new SingleInvalidation<>(observable,
                            ...
                            if (numMoved > 0) {
                                System.arraycopy(oldListeners, index + 1,
                                        invalidationListeners, index, numMoved);
                            }
                            invalidationSize--;
                            if (!locked) {
        ...
        @Override
        @SuppressWarnings("unchecked")
        protected Generic<T> addListener(ChangeListener<? super T> listener) {
            if (changeListeners.length == changeSize) {
                changeListeners = Arrays.copyOf(changeListeners,
                        changeSize + (changeSize >> 1) + 1);
            }
                ...
                for (int index = 0; index < changeSize; index++) {
                    if (listener.equals(changeListeners[index])) {
                        if (changeSize == 1) {
                            ...
                            return new SingleChange<>(observable,
                            ...
                            if (numMoved > 0) {
                                System.arraycopy(oldListeners, index + 1,
                                        changeListeners, index, numMoved);
                            }
                            changeSize--;
                            if (!locked) {
