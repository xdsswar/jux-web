...
import xss.it.jux.reactive.collections.internal.ChangeHelper;
...
/**
 * Package-private builder that accumulates list mutation operations and constructs
 * {@link ListChangeListener.Change} objects when a {@code beginChange}/{@code endChange}
 * block completes.
 *
 * <p>This builder supports nested {@code beginChange}/{@code endChange} pairs. Only the
 * outermost {@code endChange} call fires a notification to listeners. Individual
 * {@code next*} calls record add, remove, set, update, and permutation operations which
 * are compressed and merged into the final {@code Change}.</p>
 *
...
final class ListChangeBuilder<E> {
    ...
    public void nextPermutation(int from, int to, int[] perm) {
        ...
        if ((addRemoveChanges != null && !addRemoveChanges.isEmpty())) {
            ...
            // -1 means the element was not in the original list.
    ...
    /**
     * A {@link Change} implementation that wraps a single sub-change.
     */
    ...
    /**
     * A {@link Change} implementation that wraps an array of sub-changes, iterated
     * sequentially via {@link #next()}.
     */
        ...
        public String toString() {
            ...
            while (c < changes.length) {
                if (changes[c].perm.length != 0) {
                    ...
                    b.append(ChangeHelper.addRemoveChangeToString(changes[c].from, changes[c].to,
                            getList(), changes[c].removed));
                }
