 ...
 * implementation of {@link ObservableValue}. {@code Binding}
 ...
 * <p>
 * All bindings in the JUX reactive system are calculated lazily. That means, if
...
public interface Binding<T> extends ObservableValue<T> {
     ...
     * the {@code Binding} next time it is requested.
     */
    void invalidate();

    /**
     * Returns the dependencies of a binding in an unmodifiable
     * {@link ObservableList}. The implementation is
     ...
     * <p>
     * All bindings in this implementation use direct {@code InvalidationListener}
     * references (not weak references), so you should dispose unused
     * {@code Bindings} to avoid memory leaks.
     */
    void dispose();
}
